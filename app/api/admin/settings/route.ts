...
const publicFields = ["phone", "email", "address", "whatsapp", "instagram", "bale"] as const;
...
export async function PUT(request: Request) {
  ...
  const values = Object.fromEntries(publicFields.map((field) => [field, typeof body[field] === "string" ? body[field].trim() : ""]));
  if (publicFields.some((field) => !values[field])) return NextResponse.json({ error: "تکمیل اطلاعات سایت الزامی است" }, { status: 400 });
  ...
  await connectDB();
  const settings = await SiteSettings.findOneAndUpdate({ key: "site" }, { $set: { ...values, key: "site" } }, { upsert: true, new: true, setDefaultsOnInsert: true }).lean();
